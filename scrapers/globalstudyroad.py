"""
Scraper for GlobalStudyRoad website.
...
class GlobalStudyRoadScraper(BaseScraper):
    """Enhanced scraper for GlobalStudyRoad with better parsing."""

    def __init__(self):
        super().__init__(delay_range=(2, 4), max_retries=3)
        self.base_url = "https://www.globalstudyroad.com"
        # Focus on relevant sections
        self.start_urls = [
            "https://www.globalstudyroad.com/scholarships/",
            "https://www.globalstudyroad.com/phd-opportunities/",
            "https://www.globalstudyroad.com/masters-scholarships/",
            "https://www.globalstudyroad.com/postdoctoral-opportunities/",
            "https://www.globalstudyroad.com/fellowships/"
        ]

    def scrape(self) -> List[Scholarship]:
        """Scrape scholarships from GlobalStudyRoad with enhanced parsing."""
        scholarships = []
        ...
        logger.info(f"GlobalStudyRoad scraper found {len(scholarships)} total scholarships")
        return scholarships
    ...
    def _extract_scholarships_from_page(self, soup: BeautifulSoup, page_url: str) -> List[Scholarship]:
        ...
        # Strategy 2: Look for divs with class containing post, entry, scholarship, opportunity
        if not scholarships:
            container_divs = soup.find_all('div', class_=lambda c: c and any(
                keyword in c.lower() for keyword in ['post', 'entry', 'scholarship', 'opportunity', 'listing']
            ))
            ...
            for li in list_items:
                ...
                if len(text) > 30 and any(keyword in text.lower() for keyword in
                                        ['scholarship', 'funded', 'grant', 'fellowship',
                                         'opportunity', 'fellowship', 'master', 'phd', 'deadline', 'apply']):
